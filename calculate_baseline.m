function []=calculate_download_baseline(year,month,month_day,station,usrid)
...
data(data(:,9)==999999,9)=nan;
q_data=data(ia,:);
lt=datevec(datetime(q_data(:,1:6))+minutes(q_data(1,8)*4));
lt=lt(:,4)+lt(:,5)/60+lt(:,6)/3600;
[ia,~]=find(lt>=22|lt<=2);
baseline=nanmean(q_data(ia,9));
data_rb=data(:,9)-baseline;
data=[data,data_rb];
save(['E:\Data\Ground_main\',station,'\',num2str(year),num2str(month,'%02d'),'_rb.mat'],'data')
